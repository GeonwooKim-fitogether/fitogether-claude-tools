...
| [`/goal-prompt`](.claude/commands/goal-prompt.md) | **무인 작업 지시서 생성.** `/goal`(완료 조건을 걸어 두면 달성까지 사람 없이 계속 도는 모드)에 넣을 지시서를 만들어 줍니다. 목표·작업 원칙·수행 단계·완료 기준 네 블록에 **시간 상한을 필수로** 채우고, 완료를 대화가 증명할 수 있는지와 사람 승인이 필요한 작업이 범위에 섞이지 않았는지를 먼저 확인합니다. 골을 대신 실행하지는 않습니다 — 지시서를 건네는 데까지가 이 커맨드의 일입니다. |
...
| [`judgment-placement.md`](.claude/rules/judgment-placement.md) | **판단 배치 규칙** — 사람이 할 일·AI 가 할 일·코드가 할 일을 가른다. 새 판단이 생기면 "코드가 판정할 수 있나 → AI 가 판정해도 되는 판돈인가 → 아니면 사람"의 순서로 자리를 정하고, 한 번 정한 자리는 시간이 지나며 아래로 내려간다(사람 → AI 는 위임 원장, AI → 코드는 검사기 격상). 판단 31개의 전수 배치 현황은 [부록](.claude/rules/reference/judgment-placement-map.md)에 있다. |
